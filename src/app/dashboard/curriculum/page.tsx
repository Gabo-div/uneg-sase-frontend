  ...
  return (
    <div className="w-full grid grid-cols-1">
      <Card className="flex flex-col">
        <h2 className="uppercase text-gray-600 font-semibold mb-2 dark:text-gray-300">PENSUM</h2>
        <Table>
          <TableHeader>
              ...
              <React.Fragment key={semester.number}>
                {semester.subjects.map((subject) => (
                  <TableRow key={subject.code}>
                      ...
                      <div className="flex gap-1">
                        {
                          subject.neededSubjects.map((neededSubject) => (
                            <Tooltip key={subject.code + "-" + neededSubject.code} >
                              <TooltipTrigger asChild>
                                <Badge variant="outline" className="text-gray-600 cursor-pointer">{neededSubject.code}</Badge>
                              </TooltipTrigger>
                              <TooltipContent>
                    ...
                    <TableCell>{subject.approved ? <Badge className="bg-blue-800 hover:bg-blue-800 dark:bg-transparent dark:text-zinc-300">Aprobada</Badge> : ""}</TableCell>
                  </TableRow>
                ))}
                <TableRow>
                  <TableCell colSpan={6} className="text-center text-gray-600 uppercase font-bold bg-gray-50 dark:bg-zinc-900 dark:text-zinc-400">
