...
export const groupSubjectsBySemester = (subjects: Subject[]): SemesterInfo[] => {
  ...
  for (const subject of subjects) {
    ...
    subjectsBySemester[index].subjects.push({
      ...subject,
      neededSubjects: subject.needed.map((neededSubjectCode) => subjectsMap.get(neededSubjectCode)).filter((subject) => subject !== undefined) as Subject[],
    });
  }
...
export const groupSubjectsByCategory = (subjects: Subject[]): SubjectsCategories => {
