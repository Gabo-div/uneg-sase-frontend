...
import { WithdrawData } from "@/models/inscription";

type Props = {
	withdrawData: WithdrawData;
};

export default function WithdrawForm({ withdrawData }: Props) {
	const {
		sortedSubjects,
		selectedUC,
		selectedSubjects,
		subjectsToInscribe: subjectsToWithdraw,
		resetSelectedSubjects,
		handleSubjectChange,
	} = useSubjectForm(withdrawData.availableSubjects);
	...
	const getSectionName = (section: number | undefined): string => {
		...
		if (section === 0) return "No inscribir";
	...
	return (
		...
		>
			<div className="font-bold">
				<ul>
					<li>UC Inscritas: {withdrawData.registeredUC}</li>
					...
					>
						...
						<div>
							...
							>
								...
								<SelectContent>
									<SelectItem value="0">No inscribir</SelectItem>
			...
			<div className="ml-auto space-x-2">
				...
				<WithdrawConfirmationDialog
					subjectsToWithdraw={subjectsToWithdraw}
					selectedUC={selectedUC}
				>
					<Button disabled={subjectsToWithdraw.length === 0}>Continuar</Button>
				</WithdrawConfirmationDialog>
			</div>
		</form>
	);
